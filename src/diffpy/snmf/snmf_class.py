...
class SNMFOptimizer:
    """An implementation of stretched NMF (sNMF),
    including sparse stretched NMF.

    Instantiating the SNMFOptimizer class prepares initial guesses and
    sets up the optimization. It can then be run using fit().
    ...
    npj Compu Mater 10, 193 (2024). https://doi.org/10.1038/s41524-024-01377-5
    ...
    stretch_ : ndarray
        The best guess (or while running, the current guess) for the
        stretching factor matrix.
    components_ : ndarray
    ...
    min_iter : int
        The minimum number of times to update each of stretch, components, and
        weights before terminating the optimization due to low/no improvement.
    tol : float
    ...
    num_updates : int
        ...
        and weights) have had their values changed. If not terminated by
        other means, this value is used to stop when reaching max_iter.
    objective_difference : float
        The change in the objective function value since the last update.
        A negative value means that the result improved.
    """
    ...
    def __init__(
        ...
        init_stretch : ndarray Optional  Default = np.ones((self.n_components,
                self.n_signals)) + self._rng.normal(
                0, 1e-3, size=(self.n_components, self.n_signals)
            The initial guesses for the stretching factor for each component,
            at each condition (for each signal).
            Shape is (number_of_components, number_of_signals).
        ...
        # Second-order spline: Tridiagonal (-2 on diag, 1 on sub/superdiags)
    ...
    def fit(self, rho=0, eta=0, reset=True):
        """Run the sNMF optimization with the given parameters,
        using the setup from __init__.
        ...
        eta : int Optional  Default = 0
            ...
            instability, should be used only after first selecting the
        ...
        print(
            ...
            f", Obj - reg/sparse: {self.objective_function -
                                   regularization_term - sparsity_term:.5e}"
        )
        ...
        for outiter in range(self.max_iter):
            ...
            # Conv. check: Stop if diffun small and min_iter iterations passed
            print(
                "Checking if ",
    ...
    def normalize_results(self):
        ...
        # just re-running with component updates only vs norm. weights/stretch
    ...
    def get_residual_matrix(self, components=None, weights=None, stretch=None):
        """
        Return the residuals (difference) between the source matrix and
        its reconstruction.
    ...
    def compute_stretched_components(
        ...
        """
        Interpolates each component along its sample axis according to
        per-(component, signal) stretch factors, then applies
        per-(component, signal) weights. Also computes the first and second
        derivatives with respect to stretch. Left and right, respectively,
        refer to the sample prior to and subsequent to the interpolated
        sample's position.
        ...
        Outputs
        -------
        stretched_components : array,
        shape (signal_len, n_components * n_signals)
            Interpolated and weighted components.
        d_stretched_components : array, shape
        (signal_len, n_components * n_signals)
            First derivatives with respect to stretch.
        dd_stretched_components : array, shape
        (signal_len, n_components * n_signals)
        ...
        # Apply stretching to original sample indices as a "time-stretch"
        t = (
        ...
        # For each str coordinate, find prior integer index and their diff
        ...
        comps_3d = components[
            :, :, None
        ]  # expand components by a dimension for broadcast across n_signals
        ...
        # Flatten to expected shape (signal_len, n_components * n_signals)
        return (
    ...
    def update_weights(self):
        ...
        for signal in range(self.n_signals):
            ...
            # Build stretched_comps[:, k] by interpolating component
            # at frac. pos. index / this_stretch[comp]
    ...
    def update_stretch(self):
        ...
        cache = {"x": None, "fun": None, "grad": None}
        ...
        def fun_only(stretch_vec):
            if cache["x"] is None or not np.array_equal(
                stretch_vec, cache["x"]
            ):
                fun, grad = objective(stretch_vec)
                cache["x"] = stretch_vec.copy()
                cache["fun"] = fun
                cache["grad"] = grad
            return cache["fun"]

        def jac_only(stretch_vec):
            if cache["x"] is None or not np.array_equal(
                stretch_vec, cache["x"]
            ):
                fun, grad = objective(stretch_vec)
    ...
    def _compute_objective_function(
        ...
        r"""
        Computes the objective function used in stretched
        ...
        stretch : ndarray
            Stretching factors :math:`A` applied to each component
            across samples.
        rho : float
        ...
        where :math:`Z` is the data matrix, :math:`Y` contains
        the non-negative weights, :math:`S(A)` denotes the spline-interpolated
        stretching operator, and :math:`\lVert \cdot \rVert_F` is the
        Frobenius norm.
